...
def ImagesWithCC(wrapper_img, start_date, end_date, region_of_interest=None):
    ...
    def _count_valid(img):
        ...
        # ee.Reducer.allNonZero()：返回一个缩减器，这个缩减器会将一组图像转换为单个图像，
        # 其中所有像素都为 1 表示所有输入图像在该像素位置上的像素值都为非零值，反之则为 0。
        # 即得到的缩减器可以将一组图片转为一张图片，这张图片是布尔类型的图像，所有图片对应位置都不为零的话，就标记为 1 ，只有有一个为零就为 0 ，
        # 换句话，每个像素值等于所有输入图像中对应像素位置非零值的逻辑“或”（OR）操作的结果。
    ...
    if hasattr(wrapper_img, "toa_norm_fun"):
        ...
        # toa_norm_fun就是toa_norm_s2l1c()方法，用来处理（标准化）s2l1c图像的方法
...
def SelectImagesTraining(wrapper_img,
    ...
    for lag in range(1, num_images+1):
        ...
        new_band_names = time_series_operations.GenerateBandNames(wrapper_img.all_bands(), "_lag_"+str(lag)) # 拼接两个字符串
        image_add = image_add.select(wrapper_img.all_bands(), new_band_names) # 选择所有波段和新波段
...
def CloudClusterScore(img, region_of_interest, num_images=3, method_pred="persistence",
    ...
    # 选择图片去训练，筛选出的图片波段数量是源图像的两倍，即选出来的图像带有新的由“lag”标识的波段
